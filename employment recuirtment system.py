...
label=Label(text="EMPLOYMENT RECURITMENT SYSTEM",font=("times new roman",35),bg="black",fg="white")
...
def second():
    ...
    def candidate():
        candidate1=Tk()
        candidate1.title("CANDIDATE DETAILS")
    def candidateid():
        rot = Tk()
        rot.title("RAMAN")
        rot.geometry("1600x1000+0+0")
        mainlabel = Label(rot, text="CANDIDATE DETAILS", font=("times new roman", 35), bg="black",fg="white")
        mainlabel.pack(side=TOP, fill=X)
        chat1 = ttk.Treeview(rot,height=20, columns=('name','sur','marks'), selectmode="extended")
        chat1.heading('#0', text='ID', anchor=CENTER)
        chat1.heading('#1', text=' NAME', anchor=W)
        chat1.heading('#2', text='CGPA', anchor=W)
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT id ,name, Cgpa ,  sur FROM kistar ')
    ...
    def viewenquiry2():
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT * FROM sharma')
    ...
    def distroy5():
        secondw.destroy()
        window()
    mainlabel= Label(secondw,text="EMPLOYMENT RECURITMENT SYSTEM ", font=("times new roman", 35), bg="black",fg="white")
    mainlabel.pack(side=TOP, fill=X)
    button = Button(secondw,width=15, font=("arial", 20), text="REGISTRATION", bg="black",fg="white", command=distroy4)
    button.place(x=10, y=480)
    enquiry = Button(secondw, width=15, font=("arial", 20), text="EMPLOYEE DATA", bg="black",fg="white",command=distroy5)
    ...
    viewenquiry1 = Button(secondw, width=15, font=("arial", 20), text="CANDIDATE DETAILS", bg="black",fg="white",command=candidateid)
...
def root():
    ...
    label4=Label(root,text="JOB TITLE", font=("arial",17))
    label4.place(x=300, y=450)

    label4=Label(root,text="OVERALL CGPA", font=("arial",17))
...
#==============================entryfield========================================
    ...
    entry4=Entry(root,bd=5, text="enter CANDIDATE ID",width=20,textvar=search ,font=("arial",15))
...
#===============================checkbutton====================================
    ...
    checkbutton1=Checkbutton(root,text="TESTER",variable=tester)
    checkbutton1.place(x=502,y=455 )

    checkbutton1=Checkbutton(root,text="DEVELOPER",variable=developer)
    checkbutton1.place(x=555,y=455 )

    checkbutton1=Checkbutton(root,text="ANALYST",variable=analyst)
    checkbutton1.place(x=600,y=455 ,)
	 
	 
    checkbutton1=Checkbutton(root,text="CODER",variable=coder)
...
#=========================buttton==========================
    ...
    button1=Button(root,text="SALARY DETAILS",width=14,font=("arial",10),bg="black",fg="white" ,command=calculation)
...
with conn:
    cur=conn.cursor()
   # cur.execute('CREATE TABLE IF NOT EXISTS kistar(id INTEGER primary key autoincrement ,name text,sur text,email, job title text,gander text, salary details int, coder int,tester int,developer int,analayst int)')
   # cur.execute('CREATE TABLE IF NOT EXISTS Sharma (NAME TEXT, PHONE INT ,PURPOSE TEXT,DATE)')
   # cur.execute('CREATE TABLE IF NOT EXISTS FEEINSTALLMENT (id int ,TOTEL SALARY INT, BONUS INT, INCENTIVE DETAILS INT,DATE)')
...
def ka():
    ...
    with conn:
        cur=conn.cursor()
        cur.execute('INSERT INTO Sharma(NAME,PHONE,PURPOSE,DATE)VALUES(?,?,?,?)',(NAMEE,PHONE,PURPOSE,today,))
        conn.commit()
def r():
    j()
    ka()
...
def enquiry1():
    ...
    box2['values'] = ['SELECT', ' PROGRAMMER', 'TESTER', 'SALARY DETAILS']
    box2.current(0)
...
def cat():
    ...
    with conn:
        cur=conn.cursor() 
        cur.execute('SELECT amount FROM kistar WHERE id=?',(S,))
        for row24 in cur.fetchall():
          ...
          cur.execute(' SELECT SUM(amount) FROM SALARY WHERE id=? GROUP BY id ', (S,))
          for row23 in cur.fetchall():
              entry27.delete(0, END)
              entry27.insert(0, row23)
              ye = entry27.get()
              z = int(float((entry26.get()))) - int(float((entry27.get())))
              #cur.execute('INSERT INTO SALARY(id , TOTAL,AMOUNT,PAID ,REMAIN, DATE)VALUES(?,?,?,?,?,?)',(S, x, y, ye, z, today,))
...
def salary():
    ...
    entry28.configure(state="normal")
    entry26.configure(state="normal")
    cur.execute('INSERT INTO SALARY(id , TOTAL,AMOUNT, DATE)VALUES(?,?,?,?)', (S, x,y, today,))
    cur.execute(' SELECT SUM(AMOUNT) FROM SALARY WHERE id=? GROUP BY id ',(S,))
    for row23 in cur.fetchall():
        entry27.delete(0,END)
        entry27.insert(0,row23)
        ye=entry27.get()
        z=int(float((entry26.get())))-int(float((entry27.get())))
        cur.execute('UPDATE SALARY SET PAID=? WHERE id=?' , (ye,S,))
        cur.execute('UPDATE SALARY SET REMAIN=? WHERE id=?',(z,S,))
        entry28.configure(state="normal")
        entry28.delete(0,END)
        entry28.insert(0,z)
        print(row23)
...
def SALARY_DETAILS():
    if int(entry29.index("end"))>int(0):
        ...
        x=tkinter.messagebox.showinfo("NO AMOUNT PAID","YOU HAVE NOT PAID ANY SALARY ")
...
def window():
  ...
  label3=Label(main,text="ENTER EMPLOYEE DATA ID", font=("arial",17))
  label3.place(x=400, y=100)
  label3 = Label(main, text="ENTER SALARY", font=("arial", 17))
  ...
  button23=Button(main,text="ADD DETAILS",width=26,font=("arial",10),bg="black",fg="white",command=SALARY_DETAILS )
  ...
  label31=Label(main,text="TOTAL SALARY", font=("arial",17))
  label31.place(x=900, y=550)
  label32=Label(main,text="PAID SALARY", font=("arial",17))
  label32.place(x=600, y=550)
  label33=Label(main,text="REMAINED SALARY", font=("arial",17))
...
def calculation():
 ...
 print(GANDER)
 CALCULATE = calculate.get()
 if NAME==("") and  SUR==("")and  EMAIL==("") and BOX==("SELECT") and  GANDER==(3) and  CODER==(0) and  TESTER==(0) and DEVELOPER==(0) and  ANALYST==(0):
            kal=tkinter.messagebox.showinfo(" DETAILS INVALID","FILL ALL THE DETAILS")
 
 else:
     global x
     if box.get()=="DEVELOPING DEPARTMENT" and gander.get()==0:
         x=(calculation2-calculation2*20/100)
         entry5.configure(state="normal")
         entry5.delete(0,END)
         entry5.insert(0,x)
         entry5.configure(state="disable")
     if box.get()=="DEVELOPING DEPARTMENT" and gander.get()==1:
         x=(calculation2-calculation2*10/100)
         entry5.configure(state="normal")
         entry5.delete(0, END)
         entry5.insert(0, x)
         entry5.configure(state="disable")
     if box.get()=="GRAPHICAL DEPARTMENT" and gander.get()==1:
         x=(calculation2)
         entry5.configure(state="normal")
         entry5.delete(0, END)
         entry5.insert(0, x)
         entry5.configure(state="disable")
     if box.get()=="ANALYST DEPARTMENT" and gander.get()==0:
         x=(calculation2-calculation2*10/100)
         entry5.configure(state="normal")
         entry5.delete(0, END)
         entry5.insert(0, x)
     ...
     if box.get()=="SENIOR OFFICIALS" and gander.get()==1:
         x=(calculation2)
         entry5.configure(state="normal")
         entry5.delete(0, END)
         entry5.insert(0, x)
         entry5.configure(state="disable")
     if box.get()=="SENIOR OFFICIALS" and gander.get()==0:
         x=(calculation2-calculation2*10/100)
         entry5.configure(state="normal")
         entry5.delete(0, END)
         entry5.insert(0, x)
...
def msg():
 if Jobtitle.get()=="SELECT" or  gander.get()==3 or  ( coder.get()==0 and  tester.get==0 and developer.get()==0 and analyst.get()==0):
 ...
 if  "@" and ".com" not in entry3.get() :
    ...
    if msg>0:
     ...
     with conn:
         cur=conn.cursor()
         cur.execute('INSERT INTO kistar (name,sur, email, job title, gander,qualification ,coder,tester,developer,analyst ) VALUES(?,?,?,?,?,?,?,?,?,?)',(NAME,SUR,EMAIL,JOBTITLE,GANDER,QUALIFICATION,CODER,TESTER,DEVELOPER,ANALYST,))
...
def search_candidates():
        ...
        with conn:
            cur=conn.cursor()
            cur.execute('SELECT coder FROM kistar WHERE id=?',(SEARCH,))
            for row6 in cur.fetchone():
                coder.set(row6)
            cur.execute('SELECT tester FROM kistar WHERE id=?',(SEARCH,))
            for row7 in cur.fetchone():
                developer.set(row7)
            cur.execute('SELECT developer FROM kistar WHERE id=?',(SEARCH,))
            for row8 in cur.fetchone():
                tester.set(row8)
            cur.execute('SELECT analyst FROM kistar WHERE id=?',(SEARCH,))
            for row9 in cur.fetchone():
                analyst.set(row9)
            cur.execute('SELECT candidate FROM kistar WHERE id=?',(SEARCH,))
...
def update():
    ...
    if box1>0:
     ...
     with conn:
         ...
         cur.execute('UPDATE kistar SET job title=? WHERE id=?',(JOBTITLE,SEARCH,))
         cur.execute('UPDATE kistar SET gander=? WHERE id=?',(GANDER,SEARCH,))
         cur.execute('UPDATE kistar SET coder=? WHERE id=?',(CODER,SEARCH,))
         cur.execute('UPDATE kistar SET developer=? WHERE id=?',(DEVELOPER,SEARCH,))
         cur.execute('UPDATE kistar SET tester=? WHERE id=?',(TESTER,SEARCH,))
         cur.execute('UPDATE kistar SET analyst=? WHERE id=?',(ANALYST,SEARCH,))
         conn.commit()
